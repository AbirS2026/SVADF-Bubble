################################################################################
# Recursive SV-ADF Date Stamps: Semiconductor and AI-Infrastructure Firms
################################################################################
...
OUTPUT_PDF <- file.path(OUTPUT_DIR, "chip_ai_6stock_bubbles_new.pdf")
OUTPUT_PNG <- file.path(OUTPUT_DIR, "chip_ai_6stock_bubbles_new.png")
...
compute_recursive_adf <- function(price_vector, date_vector, min_fraction = 0.10) {
  ...
  # Thresholds used by the SV-ADF date-stamping procedure.
  # Origination threshold is constant: log(n)/10.
  # Collapse screen and collapse threshold remain recursive: log(ns), log(ns)/2.
  ns_vec <- n * grid_df$s
  
  grid_df$boundary_orig <- log(n) / 10
...
detect_bubble_episodes <- function(grid_df, M, R, bridge_days) {
  ...
  while (i <= N) {
    ...
    # Collapse within bridge period: ADF < log(n)/10.
    j_start <- orig_idx + 1
    ...
    # Collapse after bridge period: log(ns) screen plus R-run below log(ns)/2.
...
plot_bubble_episodes_horizontal <- function(
  ...
  if (!is.na(orig_idx)) {
    ...
    # Orange curve: constant log(n)/10.
    tau_plot <- 1:grid_df$tau[orig_idx]
    orig_curve_df <- data.frame(
      Date = as.Date(dates[tau_plot + 1]),
      y = rep(log(n_total) / 10, length(tau_plot))
    )
    
    # Brown curve: recursive log(ns)/2 after bridge plus R-period confirmation.
...
OUTPUT_PDF <- file.path(OUTPUT_DIR, "six_stock_bubbles_tech_new.pdf")
OUTPUT_PNG <- file.path(OUTPUT_DIR, "six_stock_bubbles_tech_new.png")
...
# Origination is identified when the recursive statistic exceeds log(n)/10
...
detect_bubble_episodes <- function(grid_df, M, R, bridge_days) {
  ...
  while (i <= N) {
    ...
    # Collapse within the bridge period: ADF < log(n)/10.
    j_start <- orig_idx + 1
    j_end <- idx_bridge_end - 1
...
plot_bubble_episodes_horizontal <- function(
  ...
  if (!is.na(orig_idx)) {
    ...
    # Orange origination threshold: constant log(n)/10.
    tau_plot <- 1:grid_df$tau[orig_idx]
    orig_curve_df <- data.frame(
      Date = as.Date(dates[tau_plot + 1]),
      y = rep(log(n_total) / 10, length(tau_plot))
    ...
    # Brown collapse threshold: recursive log(ns)/2 after bridge plus R-period confirmation.
    collapse_curve_start_date <- bridge_start_date + bridge_days + R
    idx_collapse_curve_start <- which(grid_df$Date >= collapse_curve_start_date)[1]
    ...
    # Collapse inside the bridge: first crossing below log(n)/10.
    j_start <- orig_idx + 1
    j_end <- idx_bridge_end - 1
...
NASDAQ_OUTPUT_PDF <- file.path(OUTPUT_DIR, "nasdaq_two_panel_bubbles.pdf")
NASDAQ_OUTPUT_PNG <- file.path(OUTPUT_DIR, "nasdaq_two_panel_bubbles.png")

CRYPTO_OUTPUT_PDF <- file.path(OUTPUT_DIR, "crypto_two_panel_bubbles.pdf")
CRYPTO_OUTPUT_PNG <- file.path(OUTPUT_DIR, "crypto_two_panel_bubbles.png")
...
make_display_adf_series <- function(grid_df, orig_idx, compression_factor = 2) {
  grid_df$ADF_delta_plot <- grid_df$ADF_delta
...
plot_bubble_episodes_horizontal <- function(
  ...
  # Keep price curve in the lower part of the statistic panel.
  price_band_low <- left_min_plot + 0.01 * (left_max_plot - left_min_plot)
  price_band_high <- left_min_plot + 0.38 * (left_max_plot - left_min_plot)
...
################################################################################
# Figure 9: Nasdaq recursive SV-ADF date-stamps
################################################################################

out_nasdaq_recent <- plot_bubble_episodes_horizontal(
  ticker = "^IXIC",
  company_name = "NASDAQ",
...
out_nasdaq_long <- plot_bubble_episodes_horizontal(
  ticker = "^IXIC",
  company_name = "NASDAQ",
  start_date = as.Date("1982-01-01"),
  end_date = as.Date("2011-12-01"),
  L = 360,
  M = 360,
  R = 360,
  bridge_days = 720
...
p_recent <- out_nasdaq_recent$plot +
  scale_x_date(date_breaks = "6 months", date_labels = "%Y %b") +
  ggtitle("2020--2026") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 18))

p_long <- out_nasdaq_long$plot +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  ggtitle("1982--2012") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 18))

nasdaq_middle_panel <- arrangeGrob(
  grobs = list(p_recent, p_long),
  ncol = 1
)

nasdaq_left_label <- textGrob(
  "Recursive SV-ADF Statistic",
  rot = 90,
  gp = gpar(col = "#1f77b4", fontsize = 25, fontface = "bold")
)

nasdaq_right_label <- textGrob(
  "Nasdaq Index",
  rot = 270,
  gp = gpar(col = "#d62728", fontsize = 25, fontface = "bold")
)
...
figure9 <- arrangeGrob(
  grobs = list(nasdaq_left_label, nasdaq_middle_panel, nasdaq_right_label),
  ncol = 3,
  widths = c(0.04, 0.92, 0.04)
)
...
# 
# if (isTRUE(SAVE_FIGURES)) {
#   grDevices::pdf(NASDAQ_OUTPUT_PDF, width = 16, height = 14)
#   grid.newpage()
#   grid.draw(figure9)
#   grDevices::dev.off()
# }
# 
# if (isTRUE(SAVE_FIGURES) && isTRUE(SAVE_PNG_PREVIEW)) {
#   grDevices::png(NASDAQ_OUTPUT_PNG, width = 16, height = 14, units = "in", res = 300)
#   grid.newpage()
#   grid.draw(figure9)
#   grDevices::dev.off()
# }
...
################################################################################
# Figure 10: Bitcoin and Ethereum recursive SV-ADF date-stamps
################################################################################

out_btc <- plot_bubble_episodes_horizontal(
  ticker = "BTC-USD",
...
out_eth <- plot_bubble_episodes_horizontal(
  ticker = "ETH-USD",
...
crypto_middle_panel <- arrangeGrob(
  grobs = list(p_btc, p_eth),
  ncol = 1
)

crypto_left_label <- textGrob(
  "Recursive SV-ADF Statistic",
  rot = 90,
  gp = gpar(col = "#1f77b4", fontsize = 25, fontface = "bold")
)

crypto_right_label <- textGrob(
  "Crypto-coin Price",
  rot = 270,
  gp = gpar(col = "#d62728", fontsize = 25, fontface = "bold")
)

figure10 <- arrangeGrob(
  grobs = list(crypto_left_label, crypto_middle_panel, crypto_right_label),
  ncol = 3,
  widths = c(0.04, 0.92, 0.04)
)
...
# if (isTRUE(SAVE_FIGURES)) {
#   grDevices::pdf(CRYPTO_OUTPUT_PDF, width = 16, height = 14)
#   grid.newpage()
#   grid.draw(figure10)
#   grDevices::dev.off()
# }
# 
# if (isTRUE(SAVE_FIGURES) && isTRUE(SAVE_PNG_PREVIEW)) {
#   grDevices::png(CRYPTO_OUTPUT_PNG, width = 16, height = 14, units = "in", res = 300)
#   grid.newpage()
#   grid.draw(figure10)
#   grDevices::dev.off()
# }
...
################################################################################
# Additional Crypto Recursive SV-ADF Date Stamps
#
# Online Appendix Figure A.2:
# Cardano, Solana, Binance Coin, and Dogecoin.
#
# Updated threshold logic:
# - Origination threshold: constant log(n)/10.
# - Collapse screen: recursive log(ns).
# - Collapse threshold: recursive log(ns)/2.
################################################################################
...
detect_bubble_episodes <- function(grid_df, M, R, bridge_days) {
  ...
  while (i <= N) {
    ...
    # Collapse within bridge: ADF < log(n)/10.
    j_start <- orig_idx + 1
    j_end <- idx_bridge_end - 1
...
plot_bubble_episodes_horizontal <- function(
  ...
  if (!is.na(orig_idx)) {
    ...
    # Collapse within bridge: first crossing below log(n)/10.
    j_start <- orig_idx + 1
    j_end <- idx_bridge_end - 1
...
################################################################################
# Run Online Appendix Figure A.2
################################################################################
