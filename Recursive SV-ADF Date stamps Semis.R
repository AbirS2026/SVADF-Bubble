...
#
# Replication code for Figure 8 of:
# "Is There an AI Bubble? Robust Date-Stamping for Periods of Exuberance"
#
# Figure 8: Recursive SV-ADF statistics and stock prices for six leading
# semiconductor / AI-infrastructure firms:
#   NVIDIA, TSMC, Broadcom, ASML, Micron, and Palantir.
#
# Data source:
...
# - The empirical end dates are fixed for reproducibility.
...
OUTPUT_PDF <- file.path(OUTPUT_DIR, "chip_ai_6stock_bubbles.pdf")
OUTPUT_PNG <- file.path(OUTPUT_DIR, "chip_ai_6stock_bubbles.png")
...
# Origination is identified when the statistic exceeds log(ns)/10 for M
# consecutive recursive windows.
...
# A candidate collapse must first satisfy ADF_delta < log(ns), and then remain
# below log(ns)/2 for R consecutive recursive windows.
...
# remains readable when one panel contains very large transient excursions.
make_display_adf_series <- function(grid_df, orig_idx, compression_factor = 6) {
  ...
  for (rr in true_runs) {
    ...
    # Expand left while the statistic remains above the display threshold.
    expanded_start <- start_i
    while (expanded_start > 1 && above_noise_pre[expanded_start - 1]) {
      expanded_start <- expanded_start - 1
    }
    
    # Expand right while the statistic remains above the display threshold.
    expanded_end <- end_i
...
# This is used only to print an interpretable summary in the console.
detect_bubble_episodes <- function(grid_df, M, R, bridge_days) {
  ...
  while (i <= N) {
    ...
    # Collapse within bridge period: ADF < log(ns)/10.
    j_start <- orig_idx + 1
...
plot_bubble_episodes_horizontal <- function(
  ...
  if (!is.na(orig_idx)) {
    ...
    # Collapse inside bridge: first crossing below log(ns)/10.
    j_start <- orig_idx + 1
    j_end <- idx_bridge_end - 1
  ...
  # Colors match the main paper figure convention.
  adf_col <- "#1f77b4"
  boundary_col <- "#ff7f0e"
  ...
  if (!is.na(collapse_idx)) {
    ...
    # Manual adjustment for Meta retained for compatibility with the shared
    # plotting function. It does not affect the semiconductor panels.
...
# This is useful because Figure 8 is a combined six-panel figure.
