  ...
  if (!is.na(orig_idx) && orig_idx > 1) {
    ...
    if (any(above_orig_pre)) {
      ...
      if (length(compress_idx) > 0) {
        grid_df$ADF_delta_plot[compress_idx] <- grid_df$ADF_delta_plot[compress_idx] / 2
      }
    }
  }
...
# -----------------------------
# Generate the 2 outputs
# -----------------------------
out_nasdaq_recent <- get_bubble_result(
  ticker = "^IXIC",
  company_name = "NASDAQ",
...
out_nasdaq_long <- get_bubble_result(
  ticker = "^IXIC",
  company_name = "NASDAQ",
...
# plot_bubble_episodes_horizontal(
#   ticker = "^IXIC",
#   company_name = "NASDAQ",
#   start_date = as.Date("1982-01-01"),
#   end_date   = as.Date("2011-12-01"),
...
# )
# -----------------------------
# Plot styling consistent with tech/chip figures
# -----------------------------
...
# -----------------------------
# Global side labels (same colors/sizes as tech/chip)
# -----------------------------
left_label <- textGrob(
...
# Save figure
# -----------------------------
pdf("nasdaq_two_panel_bubbles.pdf", width = 16, height = 14)
grid.newpage()
grid.draw(final_plot)
dev.off()
...
# -----------------------------
# Same specifications as tech/chip figures
# -----------------------------
p_btc <- out_btc$plot +
...
# Save figure
# -----------------------------
pdf("crypto_two_panel_bubbles.pdf", width = 16, height = 14)
grid.newpage()
grid.draw(final_plot)
dev.off()
